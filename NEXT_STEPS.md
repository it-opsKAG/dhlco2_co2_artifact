...
- `generators/decision_support.py` — die "letzte Verdichtungsschicht":
  ...
  - `rank_co2_levers()` / `rank_cost_levers()`: One-at-a-time-Sensitivität je Stellvariable (Δ bei fixierter Hardware), verallgemeinert aus der bestehenden `co2_sensitivity_to_requests_per_month()`-Logik in `rdc_pareto.py`. Bewusst zwei getrennte Rankings statt eines künstlichen Gesamt-Scores — `quality_score` wirkt nachweislich nur auf Kosten, nicht auf CO2; `grid_ef`/`pv_share` nur auf CO2, nicht auf Kosten.
